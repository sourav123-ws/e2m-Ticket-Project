...
export const fetchMadWorldOrdersForEv_6098674Webhook = async (req, res) => {
  try {
    ...
    console.log(
      `✅ API push successful, storing in Supabase: ${finalOrder.Email}`
    );
    const stored = await storeEmailInSupabase(tableName, finalOrder.Email);
    if (!stored) {
      console.log(
        `⚠️ API succeeded but Supabase storage failed (duplicate): ${finalOrder.Email}`
      );
    }
